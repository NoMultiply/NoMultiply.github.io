...
/// <referencs path='power_effects.js' />
...
(function (window) {
    ...
    window.onresize = function () {
        ...
        elem.style.left = game_canvas.width / 2 - handle_size / 2 - 0.192 * game_canvas.height / 2 + 'px';
        elem.style.top = game_canvas.height / 2 - 0.108 * game_canvas.height / 2 + 'px';

        var elem = document.getElementById('shine_up');
        elem.style.left = game_canvas.width / 2 - 0.192 * game_canvas.height / 2 + 'px';
        elem.style.top = game_canvas.height / 2 - handle_size / 2 - 0.108 * game_canvas.height / 2 + 'px';
        ...
        for (var i = 0; i < keys.length; ++i) {
            if (keys[i].is_miss == 0 && keys[i].is_top_left == is_top_left && keys[i].is_vertical == is_vertical) {
                ...
                if (temp[0].type == 'long' && temp[0].tail_length > 0) {
                    $('#shine_' + directions[index]).css('display', 'block').addClass('rotate');
                    temp[0].pos = rate;
                    temp.push(index);
                    long_keys.push(temp);
                }
    ...
    window.update_all = function () {
        ...
        for (var i = 0; i < long_keys.length; ++i) {
            if (press[long_keys[i][1]] == false) {
                $('#shine_' + directions[long_keys[i][1]]).css('display', 'none').removeClass('rotate');
    ...
    window.pause = function () {
        ...
        if (status == 1) {
            ...
            else if (game_mode == 'level_mode' || game_mode == 'file_produce') {
                scheduler.pause();
            }
        }
        else if (status == 3) {
    ...
    window.on_finish = function (mode) {
        ...
        $('#rank').get(0).innerHTML = get_rank(score_unit.score, max_score);
        //console.log(get_rank(score_unit.score, max_score));
        ...
        $('#max_combo').get(0).innerHTML = "max combo : " + score_unit.max_combo;
        /*
    ...
    window.get_rank = function (score, max_score) {
        var rank = score / max_score;
        if (cl.goal <= 0) {
            return 'F';
        }
        if (rank > 0.95)
    ...
    window.update_progressBar = function (percent) {
        ...
        $(function () {
            ...
            if (percent >= 100 && !score_unit.progress_launch) {
                var progressbarValue = progressbar.find(".ui-progressbar-value");
                progressbarValue.css({
                    "background": "yellow"
                });
                score_unit.progress_launch = true;
            }
            else if (percent <= 0 && score_unit.progress_launch) {
                var progressbarValue = progressbar.find(".ui-progressbar-value");
                progressbarValue.css({
                    "background": "lightblue"
                });
                score_unit.progress_launch = false;
            }
        });
    }
